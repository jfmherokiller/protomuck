...
int         process_mccp2            ( DESCRIPTOR_DATA *d );
        ...
        switch (src[i])
        {
            ...
            case IAC:
                *pto = 0;
                ...
                if (*d->mth->terminal_type == 0)
                {
                    StringCopy(val,d->mth->terminal_type);
                }
                else
                {
...
int process_do_mssp( DESCRIPTOR_DATA *d, unsigned char *src, int srclen )
{
    ...
    cat_sprintf(buffer, "%c%s%c%s", MSSP_VAR, "NAME",              MSSP_VAL, tp_muckname); // Change to your Mud's name
...
//	cat_sprintf(buffer, "%c%s%c%s", MSSP_VAR, "CONTACT",           MSSP_VAL, tp_reg_email);
...
//	cat_sprintf(buffer, "%c%s%c%s", MSSP_VAR, "FAMILY",            MSSP_VAL, "MUCK");
...
//	cat_sprintf(buffer, "%c%s%c%d", MSSP_VAR, "AREAS",             MSSP_VAL, d->mth->top_area);
//	cat_sprintf(buffer, "%c%s%c%d", MSSP_VAR, "HELPFILES",         MSSP_VAL, d->mth->top_help);
//	cat_sprintf(buffer, "%c%s%c%d", MSSP_VAR, "MOBILES",           MSSP_VAL, d->mth->top_mob_index);
//	cat_sprintf(buffer, "%c%s%c%d", MSSP_VAR, "OBJECTS",           MSSP_VAL, d->mth->top_obj_index);
//	cat_sprintf(buffer, "%c%s%c%d", MSSP_VAR, "ROOMS",             MSSP_VAL, d->mth->top_room);
...
//	cat_sprintf(buffer, "%c%s%c%d", MSSP_VAR, "SKILLS",            MSSP_VAL, MAX_SKILL);
	...
	cat_sprintf(buffer, "%c%s%c%d", MSSP_VAR, "MCP",               MSSP_VAL, 1);
	...
	cat_sprintf(buffer, "%c%s%c%d", MSSP_VAR, "PUEBLO",            MSSP_VAL, 1);
