...
int translate_telopts(DESCRIPTOR_DATA *d, unsigned char *src, int srclen, unsigned char *out, int outlen)
{
	...
	if (srclen > 0 && d->mth->mccp3)
	{
		...
		switch (inflate(d->mth->mccp3, Z_SYNC_FLUSH))
		{
			...
			case Z_OK:
				...
				if (srclen + outlen > MAX_COMMAND_LEN)
				{
					srclen = MAX_COMMAND_LEN - outlen - 1;
				}
				break;
	...
	if (d->mth->teltop)
	{
		if (d->mth->teltop + srclen + 1 < MAX_COMMAND_LEN)
		{
	...
	if (HAS_BIT(d->mth->comm_flags, COMM_FLAG_REMOTEECHO))
	{
		...
		for (cnt = 0 ; cnt < skip ; cnt++)
		{
			switch (out[cnt])
			{
				...
				case 127:
					out[cnt] = '\b';
                    notify_descriptor_raw(d->descriptor, "\b \b", 3);
					break;

				case '\n':
                    notify_descriptor_raw(d->descriptor, "\r\n", 2);
					break;

				default:
					if (HAS_BIT(d->mth->comm_flags, COMM_FLAG_PASSWORD))
					{
                        notify_descriptor_raw(d->descriptor, "*", 1);
					}
					else
					{
                        notify_descriptor_raw(d->descriptor, (char *) out + cnt, 1);
					}
					break;
			}
		}
...
int process_sb_new_environ( DESCRIPTOR_DATA *d, unsigned char *src, int srclen )
{
	char var[MAX_COMMAND_LEN], val[MAX_COMMAND_LEN];
	char *pto;
	int i;
...
int process_do_mssp( DESCRIPTOR_DATA *d, unsigned char *src, int srclen )
{
	...
	cat_sprintf(buffer, "%c%s%c%s", MSSP_VAR, "NAME",              MSSP_VAL, "BasedMUD 1.5"); // Change to your Mud's name
...
void descriptor_printf(DESCRIPTOR_DATA *d, char *fmt, ...) {
    ...
    notify_descriptor_raw(d->descriptor, buf, size);
}
